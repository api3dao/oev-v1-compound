...
export const EmodeModalContent = ({ mode }: EmodeModalContentProps) => {
  ...
  const { chainId: connectedChainId } = useWeb3();
  ...
  if (emodeTxState.success) return <TxSuccessView action={<div>Emode</div>} />;
  return (
    <>
      ...
      <TxModalDetails gasLimit={gasLimit}>
        {!showModal && (
          <Row
            caption={<div>E-Mode category</div>}
        ...
        <Row
          caption={<div>Available assets</div>}
          captionVariant="description"
          mb={4}
        ...
        {showMaxLTVRow && (
          <Row
            caption={<div>Maximum loan to value</div>}
            captionVariant="description"
            mb={4}
            align="flex-start"
          >
